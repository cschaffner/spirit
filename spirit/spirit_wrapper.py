...
def api_token_from_code(request, code):
    ...
    url = u'{0}/oauth2/token/?client_id={1}&client_secret={2}&code={3}&grant_type=authorization_code&redirect_uri={4}'.format(
        settings.TOKEN_URL, settings.CLIENT_ID, settings.CLIENT_PWD, code, settings.REDIRECT_URI)
    r = requests.get(url)
...
def api_post(access_token, url, dict):
    session = requests.Session()
...
def api_swissround_final(tournament_id, round_number):
    url = '{0}/v1/swiss_rounds/?tournament_id={1}&round_number={2}&fields=%5Bgames%5D'.format(settings.HOST,
                                                                                              tournament_id,
                                                                                              round_number)
    response = session.get(url)
    response_dict = response.json()
...
def api_swissroundinfo_roundonly(tournament_id, round_number=None, ordered=False):
    if round_number is None:
        if ordered:
            url = '{0}/v1/swiss_rounds/?tournament_id={1}&fields=%5Bround_number%5D&order_by=%5Bid%5D'.format(
                settings.HOST, tournament_id)
        else:
            url = '{0}/v1/swiss_rounds/?tournament_id={1}&fields=%5Bround_number%5D'.format(settings.HOST,
                                                                                            tournament_id)
    else:
        url = '{0}/v1/swiss_rounds/?tournament_id={1}&fields=%5Bround_number%5D&round_number={2}'.format(settings.HOST,
                                                                                                         tournament_id,
                                                                                                         round_number)
    response = session.get(url)
    return response.json()
...
def api_swissroundinfo(tournament_id, round_number=None, ordered=False):
    if round_number is None:
        ...
        url = '{0}/v1/swiss_rounds/?tournament_id={1}&round_number={2}'.format(settings.HOST, tournament_id,
                                                                               round_number)
    response = session.get(url)
    return response.json()
...
def api_gamesbytournament_restr(tournament_id, offset=0):
    url = '{0}/v1/games/?limit=200&tournament_id={1}&order_by=%5Bid%5D&fields=%5Bteam_1%2Cteam_1_id%2Cteam_2%2Cteam_2_id%2Cgame_site%2Cstart_time%2Cid%2Ctournament%5D&'.format(
        settings.HOST, tournament_id)
    if offset > 0:
...
def api_deletespirit(access_token, score_id):
    session = requests.Session()
    session.headers.update({'Authorization': 'bearer {0}'.format(access_token),
                              'Content-Type': 'application/json',
                              'Accept': 'application/json'})
